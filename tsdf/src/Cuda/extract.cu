...
struct FullScan6Slice
{
  ...
  // fetch the tsdf data and the weight value
  ...
  // extract cloud from tsdf volume data
  __device__ __forceinline__ void
  operator () () const
  {
          ...
          if (x + 1 < VOLUME_X)
          {
            ...
            if (Wn != 0 && Fn != 1.f)
              if ((F > 0 && Fn < 0) || (F < 0 && Fn > 0))
              {
				...
				// find the crossing point by linearly interpolating these two points
