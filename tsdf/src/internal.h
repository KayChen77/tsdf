...
// a uptriangle matrix that records information on Jt * J (a symetry matrix), Jt * r and residual on SE3
struct JtJJtrSE3
{
...
// Evo variant
void integrateTsdfVolumeOmni(const PtrStepSz<unsigned short>& depth_raw, const float3& volume_size,
...
// get the rgb residual, result savec to count (number of correspondence, with depth difference smaller than maxDepthDelta) sigmasum (sum of square error of depth) 
// only points with large gradients will be used (>= minScale)
